...
using System.Text;
using System.Data;
using ExcelUpload.Data.Configuration;
...
namespace ExcelUpload.Data.Bases
{
        ...
        where TDataAdapter : IDbDataAdapter, new()
    {
        ...
        private string _executingCommandType;
        //private long _executingStartTicks;
        ...
        /// <typeparam name="T">Type to return</typeparam>
        /// <returns>Stored procedure call result</returns>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes", Justification = "Exception caught and swallowed")
        , System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Performance", "CA1804:RemoveUnusedLocals", MessageId = "ex", Justification = "Exception caught and swallowed")
        , System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Performance", "CA1804:RemoveUnusedLocals", MessageId = "ex2", Justification="Exception caught and swallowed")]
        protected T ExecuteScalar<T>() 
        {
            ...
            {
                _executingCommandType = "ExecuteScalar";
                InstrumentBeforeCall(command);
                value = command.ExecuteScalar();
                AfterExecute();
                command.Parameters.Clear();
                InstrumentAfterCall(command);
            }
            if (value != null)
            {
                try
        ...
        protected DataSet ExecuteDataSet()
        {
            ...
            using (IDbCommand command = this.PrepareCommand())
            {
                _executingCommandType = "ExecuteDataSet";
                InstrumentBeforeCall(command);
        ...
        protected virtual IDataReader ExecuteReader()
        {
            ...
            using (IDbCommand command = this.PrepareCommand())
            {
                _executingCommandType = "ExecuteReader";
                InstrumentBeforeCall(command);
        ...
        protected int ExecuteNonQuery()
        {
            ...
            using (IDbCommand command = this.PrepareCommand())
            {
                _executingCommandType = "ExecuteNonQuery";
                InstrumentBeforeCall(command);
        ...
        private void InstrumentBeforeCall(IDbCommand command)
        {
            Log.Debug("Before {0}, CommandTimeout={1}, CommandText={2}", _executingCommandType, command.CommandTimeout, command.CommandText);
        }

        private void InstrumentAfterCall(IDbCommand command)
        {
            Log.Debug("After {0}, CommandText={1}", _executingCommandType, command.CommandText);
        }
