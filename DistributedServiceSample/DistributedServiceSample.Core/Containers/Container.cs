...
namespace DistributedServiceSample.Core.Containers
{
    /// <summary>
    ...
    {
        private static readonly IUnityContainer ContainerInstance;

        static Container()
        {
            ContainerInstance = new UnityContainer();
            ContainerInstance.AddNewExtension<Microsoft.Practices.Unity.InterceptionExtension.Interception>();
            if (ConfigurationManager.GetSection("unity") != null)
            {
                ContainerInstance.LoadConfiguration();
            }
        }
        ...
        public static T Get<T>()
        {
            ...
            try
            {
                implementation = ContainerInstance.Resolve<T>();
            }
            catch (ResolutionFailedException ex)
            {
                //thrown if no implementations registered
                Log.Trace("Container.Get<>() - resolution failed for type: {0}, message: {1}", typeof(T).FullName, ex.Message);
            }
        ...
        public static IEnumerable<T> GetAll<T>() where T : class
        {
            ...
            implementations.AddRange(ContainerInstance.ResolveAll<T>());
        ...
        {
            if (ContainerInstance.IsRegistered<T>()) return;
            ContainerInstance.RegisterType<T>(GetLifetimeManager(lifetime), new InjectionConstructor());
            ContainerInstance.Configure<Microsoft.Practices.Unity.InterceptionExtension.Interception>()
        ...
        {
            if (ContainerInstance.IsRegistered<TInterface>()) return;
            ContainerInstance.RegisterType(typeof (TInterface), typeof (TImplementation), GetLifetimeManager(lifetime),
